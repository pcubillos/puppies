...
def guess(data, mask=None, yxguess=None):
    ...
    if yxguess is None:
        ycenter, xcenter = np.unravel_index(
            np.nanargmax(data*mask), np.shape(data))
    else:
...
def fit(data, background=None, fitbg=False, fitguess=None,
    ...
    # The fit:
    p, cov, info, mesg, success = so.leastsq(
        residuals, params, tuple(args), full_output=True)
    return p
...
def residuals(params, ny, nx, data, mask, weights):
    ...
    sigma = params[2:4]
    height = params[4]
    model = gaussian(size, center, sigma, height)
