    ...
    if(rprs != 0){
      ...
      if(z>(1-rprs) && z<=(1+rprs)){
          ...
          I1star = 1 - c1*(1-4/5.0*sqrt(sqrt(x)))        \
                     ...
                     - c4*(1-4/8.0*x);
          INDd(y,i) = 1 - I1star*(rprs*rprs*acos((z-1)/rprs) \
...
PyDoc_STRVAR(mandeltrmod__doc__,
...
static struct PyModuleDef moduledef = {
  PyModuleDef_HEAD_INIT, "mandeltr", mandeltrmod__doc__, -1, mandeltr_methods
};
...
void init_mandeltr(void){
  Py_InitModule3("mandeltr", mandeltr_methods, mandeltrmod__doc__);
  import_array();
}
#endif
